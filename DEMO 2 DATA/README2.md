...
Place `run_notepad_di_tests.bat` alongside the other files and double-click
it, or run from a command prompt:

```
run_notepad_di_tests.bat
run_notepad_di_tests.bat --large-files
run_notepad_di_tests.bat --case DI-09
```

It creates a `venv\` folder on first run, installs `requirements.txt` into
it, then executes `notepad_di_validator.py`, writing the report to
`notepad_di_report.json` next to the script. Any arguments you pass are
forwarded to the validator. Re-running reuses the existing venv.
